...
def cross_validation(K_fold, examples, dataset, neighbor_sample_size, hop, r):
    ...
    for t in aggregator_types:
        count = 1
        temp = {'dataset': dataset, 'aggregator_type': 'concat', 'avg_auc': 0.0, 'avg_acc': 0.0, 'avg_f1': 0.0,
                'avg_aupr': 0.0,'avg_p': 0.0, 'avg_r': 0.0,}
        for i in reversed(range(0, K_fold)):
            ...
            temp['avg_p'] = temp['avg_p'] + train_log['test_p']
            temp['avg_r'] = temp['avg_r'] + train_log['test_r']
        for key in temp:
