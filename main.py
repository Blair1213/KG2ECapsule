...
from models import KG2ECapsule
from config import ModelConfig, PROCESSED_DATA_DIR,  ENTITY_VOCAB_TEMPLATE, \
    ...
    DRUG_VOCAB_TEMPLATE,RELATION_VECTOR_TEMPLATE, B_MATRIX
...
def train(train_d,dev_d,test_d,kfold,dataset, neighbor_sample_size, embed_dim, n_depth, l2_weight, routings, lr, optimizer_type,
    ...
    config.exp_name = f'KG2ECapsule_{dataset}_neigh_{neighbor_sample_size}_embed_{embed_dim}_depth_' \
    ...
    auc, acc, p, r, f1,aupr, fpr, tpr = model.score(x=[valid_data[:, :1], valid_data[:, 1:2], valid_data[:, 2:3]], y=valid_data[:, 3:4])
    ...
    model.load_best_model()
    auc, acc, p, r, f1,aupr, fpr, tpr = model.score(x=[test_data[:, :1], test_data[:, 1:2], test_data[:, 2:3]], y=test_data[:, 3:4])
