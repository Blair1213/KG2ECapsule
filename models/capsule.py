...
class KG2ECapsule(BaseModel):
    def __init__(self, config):
        super(KG2ECapsule, self).__init__(config)
    ...
    def build(self):
        ...
        drug_embedding = entity_embedding(input_drug_two)
        ...
        drug1_capsule_embedding = Lambda(lambda x: self.capsule(x[0],x[1]),name="capsule1")([drug1_squeeze_embed, drug_relation_embedding])
        drug2_capsule_embedding = Lambda(lambda x: self.capsule(x[0], x[1]), name="capsule2")(
            [drug2_squeeze_embed, drug_relation_embedding])

        drug_drug_score = Lambda(
            lambda x: K.sigmoid(K.sum(x[0] * x[1], axis=-1, keepdims=True))
         )([drug1_capsule_embedding, drug2_capsule_embedding])
